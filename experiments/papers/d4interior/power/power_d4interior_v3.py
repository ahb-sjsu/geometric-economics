#!/usr/bin/env python3
"""Power and size simulation for prereg-d4interior-v3. Outcomes are never read.
...
The estimator is the convex Newton solver of `analysis/kappa_mle.py`, not the
Powell search prereg-d4interior-v2 used. That matters for the bars. On synthetic
data with a known chirality of +0.4438, Newton returns +0.4532 from every start
with a gradient norm near 1e-14, while Powell returns +0.5889 and +0.6274 from
two starts at a worse likelihood. Most of the "+0.092 bias" the v2 simulation
reported was Powell failing to reach the optimum, so every bar is recomputed here
and none is carried over.
...
from c13k_rows import build, kappa_terms, TERM_NAMES  # noqa: E402
...
def run(reps, seed, alts):
    ...
    T = kappa_terms(d, q)
    X7 = K.design(dEV, dSD, d, q)   # the convex design the Newton solver uses
    dq = d * q
    out = {
        "n_rows": int(len(d)),
    ...
    def sample(c_dq):
        vals, nonconv = [], []
        for _ in range(reps):
            th, info = K.fit(X7, draw(c_dq))
            if not info["converged"]:
                nonconv.append(float(info["grad_inf_norm"]))
                continue
            vals.append(float(th[3]))
        return np.array(vals), nonconv

    null, nc_null = sample(0.0)
    T_pos = float(np.quantile(null, 0.95))   # one-sided, the sign is predicted
    out["null_nonconverged"] = {"count": len(nc_null),
                                "grad_norms": nc_null[:5],
                                "n_converged": int(len(null))}
    ...
    for x in alts:
        ...
        out["power"][str(x)] = {
            ...
            "n_converged": int(len(v)),
            "n_nonconverged": int(len(nc)),
            "nonconverged_grad_norms": nc[:5],
        }
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(HERE, "power_d4interior_v3.json"))
    a = ap.parse_args()
