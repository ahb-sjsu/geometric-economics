#!/usr/bin/env python3
"""Confirmatory fit for prereg-d4stability-v2. Run only after the seal.

v1 was void because its design was oriented by declaration order against an
outcome oriented by risk. v2 loads `stability_rows_v2`, where every row is
oriented risky-minus-safe so `dSD > 0` by construction, and where the builder
refuses to write a corpus in which that fails. `bEV` is recorded here and the
grader voids the run if it is not positive.
...
It computes no verdict. `d4stab_grade_v2.py` holds every threshold and is the
only place a pass or fail is decided.

    python d4stab_fit_v2.py [n_permutations]
"""
...
from stability_rows_v2 import N_FOLDS, load_covariates, load_outcome  # noqa: E402
...
def main():
    ...
    assert bool((dSD > 0).all()), "rows are not risk-oriented; refusing to fit"

    # the whole corpus in one fit, as a reference point rather than a prediction
    c2_all, se_all, info_all = fit_fold(dEV, dSD, d, q, y)

    # bEV, recorded so the grader can void the run on its sign. More expected
    # value must make an option more attractive. A negative bEV is the symptom
    # that v1's orientation defect produced and that nothing in v1 looked at.
    from stability_stat import _z
    from kappa_mle import design as _design, fit as _fit
    _theta, _info = _fit(_design(_z(dEV), _z(dSD), d, q), y)
    bEV = float(_theta[0])
    ...
    print("bEV = %+.6f   min dSD = %.6f" % (bEV, dSD.min()))
    print()
    ...
    with open(os.path.join(HERE, "results_stability_v2.json"), "w",
              encoding="utf-8") as fh:
        json.dump(out, fh, indent=2)
    print()
    print("written results_stability_v2.json")
