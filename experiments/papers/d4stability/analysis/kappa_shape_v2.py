...
**This is not part of `prereg-d4stability-v2` and changes no verdict in it.** The
...
cannot see that. `prereg-d4stability-v2` measured the resulting inflation: its
...
def null_q_mean():
    """The design effect, READ from the confirmatory run rather than assumed.

    Under a correctly specified model the random-split null on Cochran's Q has a
    mean of 3, since Q is a chi-square on three degrees of freedom when the
    standard errors are right. The observed null mean is larger because trials
    are clustered about seventeen to a problem and a trial-level logistic
    likelihood cannot see that. The ratio is the variance inflation.
    """
    with open(RESULTS, encoding="utf-8") as fh:
        return float(json.load(fh)["null_Q_mean"])
...
def main():
    ...
    describe("peterson2021using, %d individual description choices, "
             "risk-oriented (v2)" % len(y), theta, se, d, q, infl)
    assert theta[0] > 0, (
        "bEV is not positive; the design is not oriented correctly and nothing "
        "below should be read")

    out = {
        "source": "EXPLORATORY, not part of prereg-d4stability-v2",
    ...
    with open(os.path.join(HERE, "kappa_shape_v2.json"), "w",
              encoding="utf-8") as fh:
        json.dump(out, fh, indent=2)
    print()
    print("written kappa_shape_v2.json")
