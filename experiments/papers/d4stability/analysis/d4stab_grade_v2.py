#!/usr/bin/env python3
"""Grader for prereg-d4stability-v2. Written BEFORE any fit touched the choices.
...
Two VOID conditions are new in v2 and are why v2 exists. F5 requires bEV > 0.
F6 requires the pipeline invariants of `pipeline_invariants.py` to have passed,
including its I0 self-test that the suite rejects the v1 orientation.
...
the line to the designed grid and a study that has to be run.
    ...
    python d4stab_grade_v2.py results_stability_v2.json
"""
...
# F5, NEW IN v2 AND THE REASON v2 EXISTS. More expected value must make an option
# more attractive. `prereg-d4stability-v1` was void because its design matrix was
# oriented by declaration order against an outcome oriented by risk, which
# reversed the risk term on 74.8 percent of rows. The single cheapest symptom was
# a negative coefficient on the expected-value difference, `bEV = -0.3251`, and
# nothing in v1 looked at it. A registration that cannot detect its own design
# being reversed is not sealed against much, so this is a VOID condition and not
# a prediction.
BEV_MUST_BE_POSITIVE = True

# F6, the pipeline invariants must have passed. `pipeline_invariants.py` mutates
# the representation of the data in ways whose effect on the answer is known, and
# its I0 self-test confirms it rejects the v1 orientation before it is trusted to
# accept anything.
INVARIANTS = "pipeline_invariants.json"
...
def main() -> int:
    path = sys.argv[1] if len(sys.argv) > 1 else "results_stability_v2.json"
    with open(path, encoding="utf-8") as fh:
        r = json.load(fh)

    grads = list(r["grad_by_fold"]) + [r["grad_whole_corpus"]]
    void_grad = (max(grads) > GRAD_TOL) or (not r["converged_all_folds"])

    bev = r.get("bEV")
    void_bev = BEV_MUST_BE_POSITIVE and not (bev is not None and bev > 0)

    inv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), INVARIANTS)
    try:
        with open(inv_path, encoding="utf-8") as fh:
            inv = json.load(fh)
        void_inv = not inv.get("all_pass", False)
        inv_checks = inv.get("checks")
    except (OSError, ValueError):
        void_inv, inv_checks = True, "invariant report not readable"

    void = void_grad or void_bev or void_inv
    ...
    out = {
        ...
        "bEV": bev,
        "min_dSD": r.get("min_dSD"),
        "F5_bEV_positive": {"bar": "bEV > 0", "value": bev, "pass": not void_bev},
        "F6_pipeline_invariants": {"checks": inv_checks, "pass": not void_inv},
    }

    if void:
        why = []
        if void_grad:
            why.append("F3, gradient tolerance not met")
        if void_bev:
            why.append("F5, bEV is %s and must be positive; a negative "
                       "expected-value coefficient says people choose against "
                       "expected value, which is the v1 defect's symptom" % bev)
        if void_inv:
            why.append("F6, the pipeline invariants did not all pass")
        out["verdict"] = "VOID under " + "; ".join(why)
    else:
    ...
    with open("grade_stability_v2.json", "w", encoding="utf-8") as fh:
