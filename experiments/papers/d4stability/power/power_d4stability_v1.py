#!/usr/bin/env python3
"""Power and size for prereg-d4stability-v1, and the seal hash.

**This script cannot read the choices.** It loads `rows_covariates.npz`, which
holds no outcome column. The choices live in `rows_outcome.npz` and are opened
...
lineage transcribed six coefficients and got five of them wrong.
    ...
    python power_d4stability_v1.py            # power and size
    python power_d4stability_v1.py --hash     # seal the bundle
"""
...
from stability_rows import load_covariates  # noqa: E402
...
N_JOBS = 6
TRUE_SPREADS = (0.0, 0.10, 0.20, 0.30, 0.40)
...
BUNDLE = [
    ...
    ("analysis", "stability_rows.py"),
    ("analysis", "stability_stat.py"),
    ("analysis", "d4stab_fit_v1.py"),
    ("analysis", "launch_fit.sh"),
    ("analysis", "d4stab_grade_v1.py"),
    ("power", "power_d4stability_v1.py"),
    (".", "prereg-d4stability-v1.md"),
]
...
def main():
    ...
    with open(os.path.join(HERE, "power_d4stability_v1.json"), "w",
    ...
    print()
    print("written power_d4stability_v1.json")
...
def do_hash():
    ...
    **The grader is inside the bundle.** v3 kept its grader out, because the
    ...
    with open(os.path.join(root, "power", "power_d4stability_v1.json"),
              encoding="utf-8") as fh:
        pw = json.load(fh)
    ...
    seal = {
        ...
        "title": "Is the Interior Chirality Stable Inside One Corpus?",
        "level": 1,
    ...
    with open(os.path.join(root, "prereg-d4stability-v1.sha256"), "w",
    ...
    print("written prereg-d4stability-v1.sha256")
