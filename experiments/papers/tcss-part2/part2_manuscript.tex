...
Encoding lotteries and $110$ published normal-form games in a common (expected value, dispersion) space, a metric fit \emph{only} on risky choice predicts strategic play, and a polar decomposition into a scale-invariant \emph{aversion angle} plus a domain-specific magnitude raises transfer to $97$--$100\%$ of a native fit \emph{in both directions}.
...
First, matrix games exercise payoff and strategic uncertainty, not the fairness/identity coordinates GDT posits; the transfer we demonstrate is of the \emph{risk} part of the metric, and social-coordinate transfer requires social-preference games (Section~\ref{sec:corroboration} supplies corroboration but not the same transfer test).
Second, level-$k$ beliefs do not help: iterated best-response encodings \emph{degrade} the native game fit, and the naive level-$0$ belief predicts best, consistent with the level-$0$ literature~\cite{wright2017}; the metric transfers because it captures the non-strategic risk response that dominates human play, not sophisticated reasoning.
...
The cross-domain test of Section~\ref{sec:transfer} exercises the risk part of the metric.
The social coordinates the framework posits are corroborated---not by the same transfer test, but by independent, large-sample evidence from a literature that never set out to test this program.
...
Two further gates remain open and are stated as such: held-out confirmation of the loss-domain reflection encoding (the standing bound of Section~\ref{sec:lowrank}), and a domain in which the metric makes a novel, quantitative, pre-registered prediction that is then confirmed rather than a re-description of a known regularity.
None of these is asserted here; they are the work the present results are pointed at.
...
The cross-domain transfer is of the risk part of the metric, and is symmetric only after the angle-lock; the raw transfer is one-directional, and reported as such.
