...
\end{tabular}
\begin{flushleft}
\footnotesize{Errors are mean~$\pm$~s.d.\ across folds. \rev{[TODO: since all models are evaluated on the same folds, report the \emph{paired} per-fold differences (mean $\pm$ s.d.\ of GDT$-$competitor across the $20$ folds, or a paired $t$/Wilcoxon), which is the correct comparison; marginal $\pm$s.d.\ columns overstate uncertainty of the difference.]} The geometric model beats expected value out of sample and lands within one standard deviation of the domain-specialized CPT with one fewer parameter.}
\end{flushleft}
\end{table}

The domain-general geometry beats expected value, \rev{matches} linear random-utility (a margin within fold noise), and ties the domain-specialized champion with fewer parameters.
% CUT: "edges out linear random-utility" -- 0.124 vs 0.128 with overlapping
% fold s.d. is a tie until the paired-fold test says otherwise.
...
\rev{\bibitem{plonsky2019}
[TODO: verify and complete --- the CPC18 competition results / BEAST-family paper (Plonsky, Apel, Ert, Tennenholtz, Bourgin, Peterson, Reichman, Griffiths, Russell, Carter, Cavanagh, Erev, ``Predicting human decisions with behavioral theories and machine learning,'' arXiv:1904.06866, or the appropriate published version). Cited in Section~\ref{sec:transfer}-C for the leaderboard positioning.]}
