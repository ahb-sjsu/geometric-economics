...
$D_4$ dihedral symmetry of the fourfold pattern & At the corners the rotation-breaking $E$ component is significant ($\chi^2=75.4$, $p=4	imes10^{-17}$) and the free $V_4$ model beats the $D_4$-constrained model on BIC; the structure is $V_4$ (Klein four), a rectangle, not a square. An earlier version rejected $D_4$ on the ground that the rotation-covariant term vanishes in the interior. That estimate was pooled over seventeen corner rows, and the unpooled interior chirality is neither zero nor stable, moving from $+0.23$ to $-0.50$ across the stake quartiles of a single corpus of 95{,}748 individual choices. The rejection rests on the corner term alone. \\
