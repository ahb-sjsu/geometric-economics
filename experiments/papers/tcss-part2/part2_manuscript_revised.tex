...
% comments beginning "% CUT:". Accept all: \renewcommand{\rev}[1]{#1}
...
\rev{We report five results.
First, on the CPC18 individual-choice benchmark ($270$ problems), predicting unseen problems under cross-validation, the geometric model beats expected value, is statistically indistinguishable from linear random-utility, and lands within one standard deviation of cumulative prospect theory with one fewer parameter.
Second, encoding lotteries and $110$ published normal-form games in a common (expected value, dispersion) space, a metric fit only on risky choice predicts strategic play; a polar decomposition into a scale-invariant \emph{aversion angle} plus a domain-specific magnitude raises transfer to $97$--$100\%$ of a native fit in both directions, and the same construction transfers a social self--other coordinate between $3{,}800$ budget-line giving choices and dictator games (shape alignment $\cos=0.96$; $45$--$100\%$ of a native fit).
Third, four claims that $\Sigma$ is low-rank, hashed before data, survive on three held-out domains---social-preference games, a $19$-country prospect-theory replication, and a six-language panel---where a rank-$1$/rank-$2$ precision matrix matches the full metric, beats the diagonal restriction, and transfers at $\approx99\%$.
Fourth, a \emph{projection-gap} design constructs matched problems identical under every scalar projection but differing on one non-monetary coordinate, forcing the entire scalar class to predict no difference; run pre-registered on a panel of six language models ($216$ persona-conditioned agents, $83{,}682$ forced choices), five of seven contrasts move in the predicted direction in all six models, and the two pre-registered failures are reported.
Fifth, both failures localize to the cost-dependent temperature, which a fixed information price motivated by rational inattention repairs, improving held-out fit.
Scope is explicit: the projection-gap evidence is from language-model populations, not humans; the low-rank result is gain-domain; and the confirmatory human study these results power is future work, not a claim.}
% CUT: previous abstract body -- restructured. Changes: (i) "edges out linear
% random-utility" -> "statistically indistinguishable" (0.124 +/- 0.009 vs
% 0.128 +/- 0.010; the text itself calls the margin "within fold noise");
% (ii) result count fixed (the old "four results" listing packed two results
% into item one while the intro lists five contributions); (iii) the
% language-model status is stated in the abstract itself, since that is the
% single most important scope condition for a reviewer.
\end{abstract}
...
\end{tabular}
\begin{flushleft}
\footnotesize{Errors are mean~$\pm$~s.d.\ across folds. These marginal columns overstate the uncertainty of a \emph{difference}, because every model is scored on the same folds and the fold-to-fold spread is common to all of them. Table~\ref{tab:cpc18paired} reports the paired per-fold differences, which are the correct comparison.}
\end{flushleft}
\end{table}

\begin{table}[!t]
\renewcommand{\arraystretch}{1.15}
\caption{Paired Per-Fold Differences in Held-Out MAE, Same $20$ Folds}
\label{tab:cpc18paired}
\centering
\begin{tabular}{l r c r}
\toprule
\textbf{Contrast} & \textbf{Mean diff.} & \textbf{95\% CI} & \textbf{Folds won} \\
\midrule
GDT $-$ EV  & $-0.0253$ & $[-0.0284,\,-0.0222]$ & $20/20$ \\
GDT $-$ RUM & $-0.0042$ & $[-0.0058,\,-0.0026]$ & $18/20$ \\
GDT $-$ CPT & $+0.0059$ & $[+0.0035,\,+0.0083]$ & $2/20$ \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize{Negative favours the geometric model. Intervals are Student-$t$ on the $20$ paired differences; all three contrasts also reject at $p<10^{-3}$ under a Wilcoxon signed-rank test, and the same ordering holds on held-out negative log-likelihood. Three contrasts on two metrics are reported together and should be read as one family; no multiplicity correction is applied.}
\end{flushleft}
\end{table}

\rev{The domain-general geometry beats expected value on every fold and linear random-utility on $18$ of $20$, both intervals clear of zero. CPT leads it by a small but consistent margin, about $0.006$ MAE, with one more parameter.}
% RESOLVED 2026-09-10: the paired-fold test was run (datasets/RESULTS_cpc18_perfold.md,
% results_cpc18_perfold.csv). It settles the two readings this comment left open.
% "edges out linear random-utility" is now supported: paired diff -0.0042,
% 95% CI [-0.0058,-0.0026], 18/20 folds. And the CPT gap is NOT a tie: paired
% diff +0.0059, CI [+0.0035,+0.0083], 2/20 folds, so "within one s.d." was the
% wrong hedge and has been removed from the table note.
...
\rev{One comparison is deliberately absent and should be addressed head-on: the CPC18 competition itself was won by BEAST-family and hybrid machine-learning models~\cite{plonsky2019}, which outpredict CPT on this benchmark. We compare against parameter-matched, interpretable decision \emph{theories}, not against the competition leaderboard, because the question is whether a four-parameter portable metric is competitive with four-to-five-parameter domain theories---not whether it beats a many-mechanism forecasting model. [TODO: confirm this is the intended positioning and cite the exact BEAST/CPC18 results paper; a reviewer familiar with CPC18 will raise this within the first read.]}

\subsection{Result 2: cross-domain transfer}
We fit $(\sigma_{\mathrm{ev}},\sigma_{\mathrm{sd}},T)$ on one domain and predict the other with no re-fit, scoring by multinomial negative log-likelihood (NLL\rev{, per choice}; chance is uniform).
\rev{Throughout, ``\% of gap closed'' is $(\mathrm{NLL}_{\text{chance}}-\mathrm{NLL}_{\text{transfer}})/(\mathrm{NLL}_{\text{chance}}-\mathrm{NLL}_{\text{native}})$, so $100\%$ means transfer equals a native fit and $<0$ means worse than chance.}
...
\rev{One status note is owed: the polar decomposition was adopted \emph{after} the raw asymmetry of Table~\ref{tab:transfer} was observed, so Result~3 is an exploratory (post hoc) analysis, not a frozen prediction. Its safeguard is that nothing beyond a single scale parameter is re-fit; its confirmatory version belongs to \texttt{prereg-v2}. [TODO: if the angle-lock was in fact registered before the transfer runs, say so and cite the hash; otherwise keep this disclosure.]}
...
\rev{The search selects its winner on the same two datasets scored above, so it is model selection, not held-out confirmation; the selected form inherits the exploratory status of Result~3. [TODO: if compute permits, re-score the selected form under nested cross-validation so the form choice is not tuned on the reported numbers.]}
...
The reverse direction---a seven-game fit predicting $3{,}800$ individual choices---reaches $45\%$, the same asymmetry (richer calibration generalizes better) seen for risk\rev{; note that here, unlike the risk leg, the angle-lock does \emph{not} rescue the weak direction ($44\%\to45\%$)---the seven-game fit is simply too thin to pin the angle, and we say so rather than average the directions}.
...
We froze it, before any held-out data, as four falsifiable claims, hashed and tagged (\texttt{prereg-sigma-v1})\rev{. [TODO: a hash is verifiable only if it was posted publicly before the data runs. For every registration in this paper (\texttt{prereg-sigma-v1}, \texttt{prereg-v1}, \texttt{prereg-v2}, \texttt{prereg-coupling-v1}), state where and when the hash was published (OSF registration, timestamped repository commit, or similar), give the hash value or a resolvable link, and add this to the Data and Code Availability section below---otherwise reviewers must take the freezing on faith, which defeats its purpose.]}
\begin{itemize}
...
\end{tabular}
\begin{flushleft}
\footnotesize{H3 (rank-$1$ beats diagonal): Fraser--Nettle $0.290<0.335$; Ruggeri $0.667<0.680$. $^{*}$Ruggeri rank-$1$ is marginal at the $0.02$ line; rank-$2$ is the tighter low-rank choice (best BIC in the original search), consistent with H4. The cross-lingual leg tests coordinate universality: LaBSE axis alignment is $+0.214$ above a placebo-axis control, and a behavioral panel reproduces the projection-gap shape at $0.74$ correlation across six languages. \rev{[TODO: the cross-lingual analysis appears only in this footnote---no methods exist anywhere in the paper for it. Either add a short subsection (which six languages; how axes are extracted from LaBSE; what the placebo axis is; what the behavioral panel and its $n$ are; what ``shape $0.74$'' correlates) or move this leg to supplementary material and drop the row. As it stands it is uncheckable.]}}
\end{flushleft}
\end{table}
...
\rev{Three scope notes keep these statements honest. (i) Theorem~1 is an unpacking of the definition of $\pi$-sufficiency---we state it as a theorem for reference, not depth---and its completeness is a property of the design \emph{class}: the seven contrasts implemented below are instances, so a null on all seven would not certify $\pi$-sufficiency, only fail to refute it. (ii) Theorem~2's proof step ``higher relative cost $\Rightarrow$ lower choice probability'' is immediate for binary menus, which is the case run below; for menus of three or more options under softmax the comparator structure must be fixed, and we restrict the registered contrasts to binary forced choice accordingly. (iii) Theorem~2 assumes no active cross-term coupling $z$ to other coordinates---yet Section~\ref{sec:lowrank}'s headline is precisely that $\Sigma$ has real off-diagonal structure. These are consistent because the rank-$1$ direction concentrates on coordinates not manipulated in the designs, so $P_{zz}$ dominates for the manipulated $z$ [TODO: verify this against the fitted rank-$1$ loadings and state the loadings on the three manipulated coordinates explicitly; if any is materially coupled, the sign-invariance argument for that contrast needs the cross-term bound spelled out].}
...
\rev{Reproducibility requires the panel be identified: [TODO: name the six models with exact versions/checkpoints, decoding temperature and sampling settings, the persona-grid construction, and the full prompt templates (repository pointer is fine). ``Six models spanning a capability ladder'' is not reproducible.] Choices were elicited as output-only binary forced choice to avoid reasoning leakage, on shared infrastructure with idempotent work items.}
% CUT: the admission-controller / AIMD / preemption-tolerant plumbing detail --
% engineering internals that belong in the repository README, not the journal
% text; kept only the two facts that affect validity (output-only elicitation,
% idempotent items).
...
\rev{The $83{,}682$ choices are not independent---personas within a model share weights, so the effective independent unit is closer to the six models than to the $216$ agents. [TODO: state the unit of analysis for every statistic in Table~\ref{tab:projgap}: over what are $\Delta$ and Cohen's $d$ computed (per-agent means? per-model means?), and are standard errors clustered by model? The safest presentation is per-model effect estimates with the $6/6$ sign count as the primary cross-model statistic---which is already the paper's strongest card---with pooled agent-level $d$ demoted to descriptive. Also name the FDR procedure and family (e.g., Benjamini--Hochberg over the seven registered contrasts).]}
...
\midrule % rows below added in this revision
\rev{Loss reversal (fail)}   & \rev{sign flip predicted} & \rev{[TODO: $\Delta$]} & \rev{---} & \rev{1/6} \\
\rev{Dose-response (fail)}   & \rev{inverted-U predicted} & \rev{monotone $\uparrow$} & \rev{---} & \rev{[TODO]} \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize{$\Delta$ is the placebo-corrected choice-probability gap; $d$ is Cohen's~$d$. All five confirmed contrasts move in the pre-registered direction across both frames, with perfect $6/6$ cross-model consistency, including the weakest model---so the effect is not capability-gated. Placebos are null at full power. \rev{The two registered failures are shown below the rule so the scorecard is complete in one place (details in Section~\ref{sec:projgap}-E).}}
\end{flushleft}
\end{table}
...
\rev{[TODO: this paragraph needs its data provenance made airtight, because CPC18 is used both here and in Section~\ref{sec:transfer}. State (i) which split of CPC18 the fixed $T$ was fit on and which held-out split the ``improves held-out fit'' and BIC~$\approx140$ claims are computed on (and the $n$ behind the BIC); (ii) whether the Table~\ref{tab:cpc18} results use the old or repaired rule (if the repair post-dates them, say so and note they are conservative); and (iii) how many dose levels underlie the Spearman values---a $+1.0$ over $k$ points is only as strong as $k$, so report $k$.]}
...
$D_4$ dihedral symmetry of the fourfold pattern & The $d\!\cdot\!q$ chirality is an artifact of the coordinate's endpoints. $|d|=1$ is not a location in the plane but exactly the set of gambles with no loss branch or no gain branch, verified on all 95{,}748 rows, and there $d\!\cdot\!q$ degenerates into $\pm q$. Fitting the regimes separately, the pooled chirality of $+0.2506\pm0.0423$ becomes $-0.0303\pm0.0728$ inside mixed gambles, replicated on six independent lines and never more than 0.8 standard errors from zero. The interior term is $d^2-q^2$ at $-0.6943\pm0.0873$. The fourfold pattern is a corner phenomenon because at the corners it is a probability effect, not a rotation. A further caution: the coordinate is a property of the problem, not the trial, so trial-level intervals on these coefficients are too narrow by roughly the square root of the subjects per problem. Problem-clustered, CPC18's interior chirality is $+0.75$ with a 95\% interval of $[-0.90, +1.38]$, which includes zero. \\
...
%==============================================================================
[TODO: this section is load-bearing for a paper whose thesis is verifiability. Provide: (i) a repository with analysis code and the panel prompt templates; (ii) the pre-registration artifacts---for each of \texttt{prereg-sigma-v1}, \texttt{prereg-v1}, \texttt{prereg-v2}, \texttt{prereg-coupling-v1}, the hash value, the public location it was posted, and the posting timestamp; (iii) the derived datasets (encoded CPC18/bogota features, panel choice data) or scripts that regenerate them from the cited public sources; (iv) model identities and versions for the six-model panel.]
}%
...
\bibitem{cpc18data}
O.~Plonsky, I.~Erev, and E.~Ert, ``All raw data for CPC18: Choice prediction competition 2018,''
Zenodo, 2019, doi: 10.5281/zenodo.2571510.

\bibitem{plonsky2019}
O.~Plonsky, R.~Apel, E.~Ert, M.~Tennenholtz, D.~Bourgin, J.~C.~Peterson, D.~Reichman,
T.~L.~Griffiths, S.~J.~Russell, E.~C.~Carter, J.~F.~Cavanagh, and I.~Erev, ``Predicting human
decisions with behavioural theories and machine learning,'' \emph{Nature Hum. Behav.},
vol.~9, pp.~2271--2284, 2025, doi: 10.1038/s41562-025-02267-6.
